...
const ReportDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/incident_report/reports/${id}`);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-md max-w-3xl w-full">
        <h2 className="text-2xl font-bold mb-4 text-center">Incident Report Details</h2>
        ...
        <div className="space-y-3">
          ...
          <p className="mt-4 text-sm text-gray-500"><strong>Created At:</strong> {formatDate(created_at)}</p>
          <p className="text-sm text-gray-500"><strong>Updated At:</strong> {formatDate(updated_at)}</p>
        </div>
      </div>
    </div>
  );
