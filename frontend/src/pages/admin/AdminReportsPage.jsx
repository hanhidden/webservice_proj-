...
function AdminReportsPage() {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/incident_reports/reports/analytics");
  ...
  const handleApplyEdit = async (reportId) => {
    ...
    try {
      const url = `http://localhost:8000/api/incident_reports/reports/${reportId}?status=${encodeURIComponent(editedStatus)}`;
